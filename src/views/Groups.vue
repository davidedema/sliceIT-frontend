<template>
  ...
  <main>
    <v-col cols="12" sm="12" md="12">
      <v-card @click="goto('/groups/' + group._id), selectGroup(group._id)" v-for="group in this.groups"
          ...
          <v-row justify="end" no-gutters class="d-flex flex-wrap-none">
            <v-col cols="auto">
              <div v-if="group.members.length < 9">
                <v-avatar v-for="n in group.members.length" :key="n" class="flex-grow-1" color="#secondary" size="60"
                  style="margin-right: 15px">
                  <v-img :src="urlImg + group.members[n]"></v-img>
                </v-avatar>
              </div>
              <div v-else>
                <v-avatar v-for="n in 8" :key="n" class="flex-grow-1" color="#secondary" size="60"
                  style="margin-right: 15px">
                  <v-img :src="urlImg + group.members[n]"></v-img>
                </v-avatar>
              </div>
          ...
          <v-card-actions>
            <v-col cols="12" class="ml-auto" height="40px" width="40px">
              <v-btn @click="goto()" color="primary">
                <v-img :src="più" height="40px" width="40px" class="mr-2"></v-img>
              </v-btn>
            </v-col>
          </v-card-actions>
        </v-row>
            ...
            <v-container>
              <v-row>
                <v-col cols="12" sm="12" md="12">
                  ...
                  <v-text-field label="Descrizione" v-model="description"></v-text-field>
                </v-col>
              </v-row>
            </v-container>
          </v-card-text>
